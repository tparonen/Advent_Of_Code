...
class Program {
    ...
    terminate() {
        if (this.terminated) {
            throw new Error('Program cannot be terminated');
        }
        this.running = false;
    ...
    suspend() {
        if (this.terminated) {
            throw new Error('Program cannot be suspended');
        }
        this.running = false;
        this.suspended = true;
    }
    ...
    async __execute() {
        try {
            while (this.running && !this.terminated && !this.suspended && this.pc < this.memory.length) {
                const output = await this.processInstructions();
                if (output !== undefined) {
    ...
    async start(args = []) {
        ...
        if (this.suspended || this.terminated) {
            throw new Error('Program cannot be started');
        }
    ...
    async resume(args = []) {
        ...
        if (!this.suspended || this.terminated) {
            throw new Error('Program cannot be resumed');
        }
        this.suspended = false;
        return this.__execute();
    }
